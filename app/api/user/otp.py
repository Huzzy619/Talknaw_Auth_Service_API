...
class OTPGenerator:
    ...
    async def __init__(self, user_id, session: AnSession, **kwargs) -> None:
        self.secret_key = await self.get_secret()
        self.user_id = user_id
        self.processed_id = int(str(int(user_id))[:4])
        self.hotp = HOTP(self.secret_key, digits=6)
        self.obj = await self.get_otp_obj(user_id=self.user_id)
    ...
    async def get_otp(self):
        ...
        new_counter = self.obj.counter + 1
    ...
    async def get_otp_obj(self, user_id, **kwargs):
        stmt = select(OTP).where(user_id=user_id)
